    ...
    def fit(self, X, y):
        ...
        for i in range(self.iterations):
            ...
            for batch in batch_index:
                ...
                loss_val = 0.5 * np.mean(
                    (y_pred - batch_y) ** 2
                ) + self._regularization(self._weights)
                self.loss_vals_.append(loss_val)
                gradient = batch_X.T.dot(y_pred - batch_y) + self._regularization.grad(
                    self._weights
                )
            ...
            if i % (self.iterations / 100) == 0:
                print(
                    "\rProcess {}%, loss_val: {}".format(percentage, loss_val), end=""
                )
                percentage += 1
    ...
    def fit(self, X, y):
        ...
        for i in range(self.iterations):
            ...
            for batch in batch_index:
                ...
                loss_val = self._calc_loss(y_pred, batch_y) + self._regularization(
                    self._weights
                )
                self.loss_vals_.append(loss_val)
                gradient = self._calc_gradient(
                    batch_X, y_pred, batch_y
                ) + self._regularization(self._weights)
                self._weights -= self.learning_rate * gradient
